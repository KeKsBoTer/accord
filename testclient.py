...
def main(args):
    ...
    file1 = open(f"test{len(nodes)}.txt", "a")
    ...
    for i in range(10):
        ...
        print(f"simple_check time = {x}")
        file1.write(f"check,{tries},{x}\n")

    for i in range(10):
        print(f"juststore{i}")
        x = simple_store(nodes)
        print(f"simple storing time = {x}")
        file1.write(f"store,{tries},{x}\n")
    file1.close()
